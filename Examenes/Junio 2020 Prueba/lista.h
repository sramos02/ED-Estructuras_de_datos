...
public:
     ...
     /* 
	   Complejidad: Lineal
	   Determina justificadamente la complejidad de esta operación: El bucle recorre todas las posiciones de la lista
	   una sola vez. Como no sabemos el tamaño de la lista tomamos N como referencia. 
	...
	void estremece() {
		...
		if (!esVacia() && _prim->_sig != NULL) {
			Nodo* nuevoUlt = _prim->_sig;
			while ((_numElems%2==0 && act->_sig != _ult) || (_numElems%2==1 && act != _ult)) {
				aux = _ult->_sig;		
	...
	void imprime_inv(ostream& out) const {
	   Nodo* n = _ult;
       while (n != NULL) {
		  out << n->_elem << " ";
		  n = n->_ant;
