...
public:
	...
	void intercambiaPareja(Nodo* prim, Nodo*sec) {
		Nodo* auxPrim = prim->_ant;
		Nodo* auxSec = sec->_sig;

		prim->_ant = sec;
		sec->_sig = prim;

		sec->_ant = auxPrim;
		if(auxPrim != NULL) auxPrim->_sig = sec;
		prim->_sig = auxSec;
		if (auxSec != NULL) auxSec->_ant = prim;

		if (_prim == prim) _prim = sec; 
		if (_ult == sec) _ult = prim;

	}

	void intercambioParesImpares() {
		Nodo* act = _prim;


		while (act != NULL && act->_sig != NULL) {
			intercambiaPareja(act, act->_sig);
			act = act->_sig; //Ahora se encuentra en segunda posición
		}
	}
